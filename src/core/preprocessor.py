...
class InputNormalizer:
    ...
    # Cyrillic visual confusables not resolved by NFKD.
    # Maps characters that look like Latin letters to their Latin equivalents
    # so that "!GИ0Я3"-style obfuscation of "IGNORE" is normalized before
    # pattern matching.  Only clear 1-to-1 visual equivalences are included
    # to avoid over-broad transliteration of legitimate multilingual content.
    _CYRILLIC_CONFUSABLES: dict[str, str] = {
        # Uppercase — visually identical or near-identical to a Latin letter
        "А": "A",   # А → A
        "В": "B",   # В → B
        "Е": "E",   # Е → E
        "К": "K",   # К → K
        ...
        "У": "Y",   # У → Y
        "Х": "X",   # Х → X
        # Reversed-letter lookalikes used in leet-speak obfuscation
        "И": "N",   # И → N (backwards-N visual; also maps phonetic И→I but N is the bypass vector)
        "Я": "R",   # Я → R (backwards-R visual)
    ...
    def _normalize_unicode(self, text: str) -> Tuple[str, int]:
        ...
        Examples:
            ...
            - "!GИ0Я3" (Cyrillic homoglyph IGNORE) → "!GN0R3"
        ...
        # Apply Cyrillic visual confusables map (not covered by NFKD).
        # Counts each replacement so callers know obfuscation was present.
        cyrillic_replaced = 0
        result_chars: list[str] = []
        for ch in cleaned:
            mapped = self._CYRILLIC_CONFUSABLES.get(ch)
        ...
        # Total homoglyphs_replaced = chars removed by NFKD/control stripping
        # plus Cyrillic confusable substitutions.
        homoglyphs_replaced = (len(before) - len(cleaned) + cyrillic_replaced)
